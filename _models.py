...
def Xgboost(X_train_model_dt, y_train, X_test_model_dt, y_test):
    ...
    # Saving ROC plot to the drive
    plot_ROC(y_test=y_test[user_input._output_col],
             y_pred_prob=XGBC_random.best_estimator_.predict_proba(X_test_model_dt)[:, 1],
             model_name='Xgboost', image_dir=path)
    print("ROC plot saved to the drive!\n")

    print("Model Performance on Test Set:\n")
    print("Accuracy:\n")
    print(str(accuracy_score(y_test[user_input._output_col], XGBC_random.best_estimator_.predict(X_test_model_dt))))
    print("\nConfusion Matrix:\n")
    # print(confusion_matrix(y_test[user_input._output_col],rf_random.best_estimator_.predict(X_test_oneHotEncoded)))
    print(pd.crosstab(y_test[user_input._output_col], XGBC_random.best_estimator_.predict(X_test_model_dt),
                      rownames=['True'], colnames=['Predicted'], margins=True))
    print("\nClassification Report:\n")
    print(classification_report(y_test[user_input._output_col], XGBC_random.best_estimator_.predict(X_test_model_dt)))
    print("\nCohen Kappa:\n")
    print(cohen_kappa_score(y_test[user_input._output_col], XGBC_random.best_estimator_.predict(X_test_model_dt)))
...
def create_model(optimizer='rmsprop', init='glorot_uniform'):
    ...
    # create model
    model = models.Sequential()
    model.add(layers.Dense(8, input_dim=12, kernel_initializer=init, activation='relu'))
...
def ANN(X_train_model_dt, y_train, X_test_model_dt, y_test):
    ...
    # print(confusion_matrix(y_test[user_input._output_col],rf_random.best_estimator_.predict(X_test_oneHotEncoded)))
    # print("\n######################################################################################")
    # print(type(annc_random.best_estimator_.predict(X_test_model_dt)))
    # print(annc_random.best_estimator_.predict(X_test_model_dt).shape)
    # print(annc_random.best_estimator_.predict(X_test_model_dt))
